// post-bound-frame-unit/js/app.js
// post-bound-frame-unit 전용 (Three.js + UI + 모델 생성)
// 특징: 포스트 + 상/하부 가로재 + ㅁ자 모듈(중간 가로재 없음)
// - 'ㅁ자 모듈 높이' 입력 제거
// - 난간 높이/하부레일 시작높이로 모듈 높이 자동 계산
...
(() => {
  ...
  function wireUI() {
    ...
    if (btnPrev)
      btnPrev.addEventListener("click", () => {
        BUILD.step = BUILD.step <= 0 ? 4 : BUILD.step - 1;
        applyStepToView();
        syncStepLabel();
        syncViewButtons();
        requestRender();
    ...
    if (btnNext)
      btnNext.addEventListener("click", () => {
        BUILD.step = BUILD.step >= 4 ? 0 : BUILD.step + 1;
        applyStepToView();
        syncStepLabel();
        syncViewButtons();
        requestRender();
  ...
  function renderRailing() {
    ...
    const postInt = clampNum($("postInt")?.value, 200, 1100);
    const postW = clampNum($("postW")?.value, 20, 20);
    const postH = clampNum($("postH")?.value, 20, 50);
    ...
    // ✅ 모듈은 "상부레일 하단" ~ "하부레일 상단" 사이를 가득 채움
    // addRectFrame는 innerH(센터-센터) 기준이라 barT 보정 필요
    const moduleAreaTop = yTopRailBottom - barT / 2;
    const moduleAreaBot = yBotRailTop + barT / 2;
    const moduleH = Math.max(10, moduleAreaTop - moduleAreaBot); // 자동 계산(센터-센터)
